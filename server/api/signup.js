...
module.exports = function (req, res, next) {
    if (req.body.query) { // Checks if there is query
        ...
        pool.connect((err, client, done) => {
            ...
            client.query('SELECT * FROM player WHERE UPPER(username) = UPPER($1)', [query[0]], (err, row) => signupCallback(done, err, row, "username"));
            client.query("SELECT * FROM player WHERE UPPER(nickname) = UPPER($1)", [query[1]], (err, row) => signupCallback(done, err, row, "nickname"));
            client.query("SELECT * FROM player where UPPER(email) = UPPER($1)", [query[2]], (err, row) => signupCallback(done, err, row, "email"));
...
function signupCallback(done, err, result, value) {
    if (err) {
        console.log(err);
    }
